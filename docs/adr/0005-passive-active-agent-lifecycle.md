...
#### 6.2.1 Implementation note — MVP gate security-review remediation (post-review)

An MVP of `ToolGate`/`ApprovalStore` (`forge_agent/active/gate.py`) landed ahead of the full Phase 1 supervisor, wired into `ManualToolBuilder`. A security review found five gaps before it can guard an irreversible/outward-facing action (e.g. a Postiz social-publish tool); all five are now closed:

1. **OpenAPI-sourced tools bypassed the gate.** `OpenAPISource` gained `requires_approval: bool` (gate every operation from the source) and `approval_operations: list[str]` (gate specific operations, by `operationId` or the legacy `"METHOD /path"` key form — same dual-key convention as `route_map`). `OpenAPIToolBuilder` takes the same shared `ToolGate` as `ManualToolBuilder`, wraps gated operations identically (draft-instead-of-execute), and — mirroring the manual path — raises at build time if a gated operation has no `ToolGate` available (fail-closed). `ToolSurfaceRegistry` threads its one shared `ToolGate` into both builders.
2. **Draft arguments leaked unredacted to `config:read`.** `GET /v1/admin/approvals` now redacts `arguments` (reusing the same logic as `GET /v1/admin/config`, factored into `forge_gateway.redaction`) for callers who hold `config:read` but not `agent:approve`; a caller who can actually decide (`agent:approve`) still sees the real values.
3. **No audit trail for approve/reject.** Both routes now record an activity-feed entry (`interface="approval"`, attributing the decision to the calling principal) on every successful decision, and — when the ADR-0004 workload plane is enabled — additionally route the decision through the AgentWeave `AuditTrail` (`record_auth_check`-shaped), degrading gracefully (log, never block the decision) if that backend is unavailable.
4. **Unbounded `ApprovalStore` growth (DoS).** The store now caps total stored requests (`ApprovalStore(max_size=...)`, default 1000): at capacity, the oldest RESOLVED entries are evicted first; if every stored request is still PENDING, a new draft is refused (`ApprovalStoreFullError`, fail-closed — never silently executed ungated).
5. **Cross-replica "exactly once" was undocumented/unguarded.** `ApprovalStore` is in-memory per replica — "exactly once" holds only within a single process (documented explicitly in the `gate.py` module docstring). `deploy/helm/forge/templates/deployment.yaml` now fails the Helm render (mirroring the existing `persistence.enabled` single-writer guard) when any gated tool is configured together with `agent.replicaCount > 1` or `autoscaling.enabled` — a loud build-time error rather than a silent multi-replica double-approval risk. A Redis-backed `ApprovalStore` (mirroring `RedisConversationStore`, ADR-0003) is the real cross-replica fix and remains a tracked follow-up, not implemented here.
